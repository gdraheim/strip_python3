...
_maindir = os.path.dirname(os.path.abspath(sys.argv[0]))
_tooldir = os.path.join(os.path.dirname(_maindir), "tools")
_mirror = os.path.join(_tooldir, "docker_mirror.py")
...
class DockerStripTest(unittest.TestCase):
    ...
    def ip_container(self, testname: str = NIX) -> str:
        testname = testname or self.caller_testname()
        values = output("docker inspect "+testname)
        values = json.loads(values)
        if not values or "NetworkSettings" not in values[0]:
    ...
    #
    #
    def test_5100_run_inside_container(self) -> None:
        """ check that we can install inside a docker container """
        ...
        sx____(cmd)
        cmd = F"{docker} cp {_strip3}/strip_python3.py {testname}:/usr/bin/strip-python3"
        sh____(cmd)
        cmd = F"{docker} exec {testname} sed -i 's:/usr/bin/env python.*:/usr/bin/env {python}:' /usr/bin/strip-python3"
        sh____(cmd)
        cmd = F"{docker} exec {testname} strip-python3 --version"
        sh____(cmd)
        out = output(cmd)
        logg.info("\n>\n%s", out)
        #
        ...
        self.rm_testdir()
        #
        self.assertTrue(greps(out, "version:"))
    def test_5200_install_inside_container(self) -> None:
        ...
        sh____(cmd)
        cmd = F"{docker} cp {strip3} {testname}:/setup/"
        sh____(cmd)
        ...
        sh____(cmd)
        cmd = F"{docker} cp exectests3.py {testname}:/"
        sh____(cmd)
        cmd = F"{docker} exec {testname} make -C setup ins PYTHON3={python} PYTHON39={python}"
        sh____(cmd)
        ...
        self.assertTrue(greps(out, "strip3/strip_python3.py"))
        self.assertTrue(greps(out, "strip3/strip_ast_comments.py"))
        self.assertTrue(greps(out, "bin/strip-python3"))
        #
        out = output(F"{docker} exec {testname} bash -c 'PATH=\\$PATH:~/.local/bin;strip-python3 --version'")
        logg.info("strip-python3 --version\n%s", out)
        self.assertTrue(greps(out, "version:"))
        out = output(F"{docker} exec {testname} /root/.local/bin/strip-python3 --version")
        logg.info("strip-python3 --version\n%s", out)
        self.assertTrue(greps(out, "version:"))
        #
        ...
        self.rm_docker(testname)
        self.rm_testdir()
    def test_5210_mypy_inside_container(self) -> None:
        if not os.path.exists(DOCKER_SOCKET): self.skipTest("docker-based test")
        images = IMAGES
        imagedef = IMAGE or IMAGEDEF
        image = self.local_image(F"{images}:test_5200")
        testname = self.testname()
        testdir = self.testdir()
        docker = _docker
        package = package_tool(image)
        ...
        cmd = F"{docker} exec {testname} bash -c '{{ echo \"import strip3.strip_python3 as app\"; echo \"app.main()\"; }} > testapp.py'"
        sh____(cmd)
        cmd = F"{docker} exec {testname} bash -c '{python} -m mypy --strict testapp.py'"
        sh____(cmd)
        self.rm_docker(testname)
        self.rm_testdir()
        #
    def test_5401(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5402(self) -> None:
        self.exectest_inside_container(self.testname())
    ...
    def test_5421(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5422(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5431(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5432(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5441(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5451(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5452(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5501(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5521(self) -> None:
        self.exectest_inside_container(self.testname())
    def test_5522(self) -> None:
        self.exectest_inside_container(self.testname())
    def exectest_inside_container(self, testname: str) -> None:
        if not os.path.exists(DOCKER_SOCKET): self.skipTest("docker-based test")
        images = IMAGES
        imagedef = IMAGE or IMAGEDEF
        image = self.local_image(F"{images}:test_5200")
        ...
        cmd = F"{docker} run --detach --name={testname} {image} sleep {sometime}"
        sh____(cmd)
        test_name = testname.replace("test_5", "test_3")
        cmd = F"{docker} exec {testname} {python} /exectests3.py --with=/root/.local/bin/strip-python3 --python={python} --python3={python} {test_name}"
        sh____(cmd)
        self.rm_docker(testname)
        self.rm_testdir()
        #
...
if __name__ == "__main__":
    ...
    _o.add_option("--with", metavar="FILE", dest="src", default=_strip3,
                  help="strip3/ parts to be tested (%default)")
