...
class StripTest(unittest.TestCase):
    ...
    def test_2232(self) -> None:
        ...
        s = "{y:n}".format(**locals())
        print(s)""")
        run = sh(F"{strip} {tmp}/tmp3.py -o {tmp}/tmp.py {vv} --upgrade -6")
        logg.debug("%s %s %s", strip, errs(run.err), outs(run.out))
        # self.assertFalse(run.err)
        self.assertTrue(os.path.exists(F"{tmp}/tmp.py"))
        self.assertFalse(os.path.exists(F"{tmp}/tmp.pyi"))
    ...
    def test_2311(self) -> None:
        ...
        self.coverage()
        self.rm_testdir()
    def test_2312(self) -> None:
        vv = self.begin()
        strip = coverage(STRIP)
        tmp = self.testdir()
        text_file(F"{tmp}/test3.py", """
        import sys
        def func1(x: Any, y: Any):
            if isinstance(x, str):
                print(x)
            if isinstance(y, str):
                print(y)
    ...
    def test_2462(self) -> None:
        ...
        def func1():
            filename = 'tmp.tmp'
            with py3_open(filename, 'r', encoding='utf-8') as f:
                pass
        """)))
        self.coverage()
        self.rm_testdir()
    ...
    def test_2622(self) -> None:
        ...
        def func1() -> int:
            try:
                raise FileNotFoundError(0, "suddenly gone", "x")
            except OSError as e:
                print(badfile, e.errno)
            return 0
        """)
        ...
        def func1():
            try:
                raise OSError(errno.ENOENT, 'suddenly gone', 'x')
            except (OSError, IOError) as e:
                print(badfile, e.errno)
            return 0
        """)))
