...
def main() -> int:
    ...
    if opt.show:
        ...
        for wantedname in sorted(dir(want)):
            if "_" in wantedname and not wantedname.startswith("_"):
                wantedvalue = getattr(want, wantedname)
                if isinstance(wantedvalue, (int, bool, tuple,)):
                    logg.info("  want.%s = %s", wantedname, wantedvalue)
