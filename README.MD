...
[![Code Coverage](https://img.shields.io/badge/249%20test-98%25%20coverage-brightgreen)](https://github.com/gdraheim/strip_python3/blob/main/strip_python3.tests.py)
...
If `builtins.open` is used with an `encoding="utf-8"` parameter
then it needs to be replaced with io.open() for python2.
...
## --raise-oserror-subclasses

if `raise FileNotFoundException("foo")` is used then it gets replaced by OSError
if python3 compatibility is requested before python 3.3 (having PEP3151)

There are a dozen of subclasses of OSError that are mapped to some errno
number. FileNotFoundException maps to `errno.ENOENT`, so that the new
source code reads `raise OSError(errno.ENOENT, "foo")`.
...
If `class Result(Enum)` is used then `from enum import Enum` is replaced by local `class Enum`.
if python2 compatibility requested, as well as python3 older than 3.3.

The Enum subclass is modified as well - note however that only basic definitions on a global
level can get replaced. Dont use `auto()` or function-level declarations. Most member functions
of an `enum` have not been implemented - you can only check for `Color(3) is Color.R`.
...
The `typing.final` decorator ('@final' function) and the `typing.Final`
annotation ('Final[int]') became available in python3.9. These can be
removed altogether (and removing the typing imports as well)
